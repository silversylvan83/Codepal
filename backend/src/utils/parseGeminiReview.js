// src/utils/parseGeminiReview.js

/**
 * @typedef {{ line: number, level: string, message: string }} ReviewComment
 * @typedef {{ summary: string, comments: ReviewComment[], improvedSnippet?: string }} ParsedReview
 */

/**
 * Parse a Gemini-style markdown review into { summary, comments, improvedSnippet }.
 * @param {string} markdown
 * @returns {ParsedReview}
 */
export function parseGeminiReview(markdown) {
  const text = markdown || '';
  ...
  let summary = '';
  {
    ...
    const head = lines
      ...
      .join('\n')
      .trim();
    // strip single full-code-fence blocks if the whole head was fenced
    summary = head.replace(/^\s*`{3}.*?`{3}\s*$/gs, '').trim();
  }

  // 2) Comments from bullets under known sections
  const comments = /** @type {ReviewComment[]} */ ([]);
  const levelMap = {
    bugs: 'bug',
    fixes: 'bug',
   ...
   */
  function collectBullets(sectionName, levelHint) {
